...
seagrassPlot<-function(flounderMeanCnts,flounderSEMCnts,cover){
  plot(as.numeric(as.factor(flounderMeanCnts$Season[flounderMeanCnts$SeagrassCover %in% cover])),
     flounderMeanCnts$value[flounderMeanCnts$SeagrassCover %in% cover],
     type='n',xlab="",ylab="Number of flounder",axes=FALSE,
     ylim=c(0,100))
  axis(1,
       labels = levels(as.factor(flounderMeanCnts$Season)),
       at=unique(as.numeric(as.factor(flounderMeanCnts$Season))),
       pos=0)
  axis(2,las=1)
  cnt=0
  for(size in levels(flounderMeanCnts$SizeClass)){
    ...
    xs<-as.numeric(as.factor(subMean$Season))
    points(xs,
           subMean$value,pch=cnt+15,col=size_cols[cnt+1],cex=2)
    ...
    arrows(x0=xs,
          ...
          col=size_cols[cnt+1],lwd=2,code=3,length=0)
    cnt<-cnt+1
  }
}
...
outer_legend("top",bty='n',pch=15:18,col=size_cols,levels(flounderMeanCnts$SizeClass),ncol=4,cex=1.5,lwd=2)
```
