...
seagrassPlot<-function(flounderMeanCnts,flounderSEMCnts,cover){
  ...
  axis(1,
       ...
       at=unique(as.numeric(flounderMeanCnts$Season)),
       pos=0,
       cex.axis=2, 
       las=2)
...
par(mfrow=c(1,4), oma=c(3,0,2,1), mar=c(6,5.5,1,1),xpd=TRUE)
...
flounderfig %>%
  image_composite(seagrassPhotos,offset="+6-413")
```
```{r}
...
```{r}
flounderGLM<-glm(flounder_long$count ~ 1+ flounder_long$Season*flounder_long$seagrass_cover * flounder_long$size * flounder_long$towArea,
                   family="poisson")
```
