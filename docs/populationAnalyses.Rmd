...
shrimp_long$Season<-factor(shrimp_long$Season,
                           levels=c("Winter","Spring","Summer","Fall"))
shrimp_long$seagrass_cover<-factor(shrimp_long$seagrass_cover,
                           levels=c("bare","sparse","patchy","dense"))
shrimp_long$density<-shrimp_long$count/shrimp_long$towArea


```
```{r shrimpDistributions}
par(mfrow=c(2,2))
hist(shrimp_long$count,xlab="Counts", ylab="Number of observations", main="Distribution of raw counts")
hist(log(shrimp_long$count+1),xlab="ln(Counts)", ylab="Number of observations", main="Distribution of raw counts")
hist(shrimp_long$density,xlab="Density (counts/m^2)", ylab="Number of observations", main="Distribution of raw densities")
hist(log(shrimp_long$density+1),xlab="ln(Density)", ylab="Number of observations", main="Distribution of log-transformed densities")
```

## Log-transformed OLS

```{r shrimpLMs}
shrimpLM1<- lm(log(density+1) ~ 1,dat=shrimp_long)
shrimpLM2<-  lm(log(density+1) ~ 1+ Season,dat=shrimp_long)
shrimpLM3<-  lm(log(density+1) ~ 1+ seagrass_cover,dat=shrimp_long)
shrimpLM4<-  lm(log(density+1) ~ 1+ size,dat=shrimp_long)
shrimpLM5<-  lm(log(density+1) ~ 1+ Season + seagrass_cover,dat=shrimp_long)
shrimpLM6<-  lm(log(density+1) ~ 1+ Season + size,dat=shrimp_long)
shrimpLM7<-  lm(log(density+1) ~ 1+  seagrass_cover + size,dat=shrimp_long)
shrimpLM8<-  lm(log(density+1) ~ 1+ Season + seagrass_cover + size,dat=shrimp_long)
shrimpLM9<-  lm(log(density+1) ~ 1+ Season * seagrass_cover + size,dat=shrimp_long)
shrimpLM10<-  lm(log(density+1) ~ 1+ Season + seagrass_cover * size,dat=shrimp_long)
shrimpLM11<-  lm(log(density+1) ~ 1+ Season * size +seagrass_cover,dat=shrimp_long )
shrimpLM12<-  lm(log(density+1) ~ 1+ Season * seagrass_cover * size,dat=shrimp_long)
...
kable(shrimpAIC,"latex",booktabs=TRUE,caption="Comparison of AICc values for model selection of Poisson GLMs describing flounder abundances. The best-fitting model is the most complex one, with interactions between season, seagrass cover, flounder size, and tow area.")
```
...
```{r shrimpANOVAtable}
kable(anova(shrimpLM12),"latex",booktabs=TRUE,
      caption="Summary ANOVA table of the effects of season, seagrass cover, shrimp size, and their interactions on the density of shrimp in Duvauchelle Bay.")
```

```{r saveShrimpANOVA}
write.csv(anova(shrimpLM12),"ShrimpLMtable.csv",row.names = TRUE,quote=FALSE)
```
...
combos <- data.frame(seagrass_cover = rep(c("bare","sparse", "patchy", "dense"), each=8), 
                     ...
                     size = rep(levels(shrimp_long$size),each=4),
                     stringsAsFactors = TRUE)
combos$seagrass_cover<-factor(combos$seagrass_cover,
                              levels=c("bare","sparse", "patchy", "dense"))
combos$Season<-factor(combos$Season,
                              levels=c("Winter","Spring","Summer","Fall"))
combos$size<-factor(combos$size,levels=levels(shrimp_long$size))

## store the predicted number of shrimp and SE for each set
shrimpPreds <- cbind(combos, predict(shrimpLM12, combos,se.fit = TRUE)[1:2])
shrimpPreds$lower<-shrimpPreds$fit-shrimpPreds$se.fit
shrimpPreds$upper<-shrimpPreds$fit+shrimpPreds$se.fit
colnames(shrimpPreds)[4]<-"counts"
...
```{r setShrimpCols}
shrimpCols<-c(below20mm='#d7301f',above20mm='#fdbb84')
```


```{r shrimpCnts}
shrimpCnts<-data.frame(count=tapply(shrimp_long$count,
                     paste(shrimp_long$seagrass_cover,
                           shrimp_long$Day,
                           shrimp_long$Season,
                           shrimp_long$size),
                     sum))
shrimpCnts$SeagrassCover<-gsub("(\\w+) (\\d+.*) (\\w+) (.*$)","\\1",rownames(shrimpCnts))
shrimpCnts$Date<-gsub("(\\w+) (\\d+.*) (\\w+) (.*$)","\\2",rownames(shrimpCnts))
shrimpCnts$Season<-gsub("(\\w+) (\\d+.*) (\\w+) (.*$)","\\3",rownames(shrimpCnts))
shrimpCnts$SizeClass<-gsub("(\\w+) (\\d+.*) (\\w+) (.*$)","\\4",rownames(shrimpCnts))


shrimpMeanCnts<-multi_tapply(shrimpCnts,"count",c("SeagrassCover","Season","SizeClass"),mean)
shrimpMeanCnts$SizeClass<-factor(shrimpMeanCnts$SizeClass,
                                   levels=c(
                                     "below20mm",
                                     "above20mm"
                                   ))
shrimpMeanCnts$Season<-factor(shrimpMeanCnts$Season,
                                levels=c(
                                  "Winter",
                                  "Spring",
                                  "Summer",
...
shrimpSEMCnts<-multi_tapply(shrimpCnts,"count",c("SeagrassCover","Season","SizeClass"),sem)
shrimpSEMCnts$SizeClass<-factor(shrimpSEMCnts$SizeClass,
                                   levels=c(
                                     "below20mm",
                                     "above20mm"
                                   ))
shrimpSEMCnts$Season<-factor(shrimpSEMCnts$Season,
                                levels=c(
                                  "Winter",
                                  "Spring",
                                  "Summer",
...
```{r seagrassPlotFxn}
seagrassPlot<-function(flounderMeanCnts,flounderSEMCnts,cover,colors){
  ymin<-0
  ymax<-round(max(flounderMeanCnts$value+flounderSEMCnts$value,na.rm=TRUE)+0.5)
  ...
  for(size in levels(flounderMeanCnts$SizeClass)){
    ...
    arrows(x0=xs,
          ...
          col=scales::alpha(colors[cnt+1],0.5),
  ...
  for(size in levels(flounderMeanCnts$SizeClass)){
    ...
    points(xs,
           subMean$value,pch=cnt+15,col=colors[cnt+1],cex=3)
     points(xs,
           subMean$value,type='l',col=colors[cnt+1],lwd=2,lty=(cnt+1))
...
```{r PlotFlounderProbsFxn}
PlotFlounderProbs<-function(predictedProbs, sgrs,
                            seagrassCs, seagrassPs,
                            xaxis=TRUE,ymax=0){
  
  dat<-predictedProbs[which(predictedProbs$seagrass_cover==sgrs),]
  
  ymin<-0
  if(ymax == 0) ymax<-round(max(dat$upper,na.rm=TRUE)+0.5)
  plot(counts~as.numeric(Season),
       data=dat[which(dat$size==levels(dat$size)[1]),],
       pch=seagrassPs[1],
       cex=2,
       axes=FALSE,
       bty='L',
  ...
  lapply(1:length(levels(dat$size)),function(i){
    cvr<-levels(dat$size)[i]
    d<-dat[which(dat$size==cvr),]
    arrows(x0 = as.numeric(d$Season),
           x1= as.numeric(d$Season),
           y0=d$lower, y1=d$upper,
  ...
  lapply(1:length(levels(dat$size)),function(i){
    cvr<-levels(dat$size)[i]
    points(counts~as.numeric(Season),
       data=dat[which(dat$size==cvr),],
       pch=seagrassPs[i],
       col=seagrassCs[i],
       cex=3)
  })
...
```{r shrimpPredictedsPlot,fig.width=6.39,fig.height=4.78}

par(mfcol=c(1,4),xpd=TRUE,mar=c(2,2,2,2),oma=c(3,4,3,2))
fp<-lapply(levels(shrimpPreds$seagrass_cover),
           PlotFlounderProbs,
           predictedProbs=shrimpPreds,
           seagrassCs=shrimpCols,
           seagrassPs=15:16,
           xaxis=TRUE)

l<-outer_legend("top",
             levels(shrimpPreds$size),
             col=shrimpCols,
             pch=15:16,
             lty=1:4,
             lwd=2,
             ...
             ncol=2,
             cex=1.5)
mtext("Predicted ln(shrimp density+1)",2,outer=TRUE,line=1.75)


```


```{r makeShrimpPlots,eval=FALSE}
png("../figs/ShrimpObsPred.png", height=5.5,width=9,res = 300,units='in')

par(mfrow=c(2,4), oma=c(8.5,2,2,1), mar=c(2,5.5,1,1),xpd=TRUE)

# predicted counts
fp<-lapply(levels(shrimpPreds$seagrass_cover),
           function(seagrass, labels){
             PlotFlounderProbs(seagrass,
                               predictedProbs=shrimpPreds,
                               seagrassCs=shrimpCols,
                               seagrassPs=15:16,
                               xaxis=FALSE, ymax=2)
             mtext(seagrass,side=3,outer=FALSE,cex=1.25)
             legend("topright",labels[seagrass],
                    bty='n',cex=2)
           },
           labels=c(bare="A",sparse="B",patchy="C",dense="D")
)
mtext("Predcited ln(density+1)",2,outer=FALSE,padj=-46.5,cex=1.25)

# observed counts
seagrassPlot(shrimpMeanCnts,shrimpSEMCnts,"bare",shrimpCols)
legend("topright","E",bty='n',cex=2)
seagrassPlot(shrimpMeanCnts,shrimpSEMCnts,"sparse",shrimpCols)
legend("topright","F",bty='n',cex=2)
seagrassPlot(shrimpMeanCnts,shrimpSEMCnts,"patchy",shrimpCols)
legend("topright","G",bty='n',cex=2)
seagrassPlot(shrimpMeanCnts,shrimpSEMCnts,"dense",shrimpCols)
legend("topright","H",bty='n',cex=2)
mtext("Observed number",2,outer=FALSE,padj=-46.5,cex=1.25)

outer_legend("bottom",bty='n',
             pch=15:16,col=shrimpCols,levels(shrimpMeanCnts$SizeClass),
             ncol=2,cex=2,lwd=3,lty=1:2,pt.cex=3)
dev.off()
```
```{r getImages}
...
```{r saveShrimpFig, eval=FALSE}
shrimpfig<-image_convert(image_read("../figs/shrimpObsPred.png"), "png")
shrimpSeagrass<-image_append(c(shrimpfig,
                                 seagrassPhotos), 
                               stack = TRUE)
image_write(shrimpSeagrass,
            path = "../figs/ShrimpFigure.png",
            format = "png",
            quality=100)
```
...
```{r flounderSummaryNotFancy,fig.cap="Overview of flounder counts, not formatted to be particularly fancy, split up by season."}
...
hist(log(flounder_long$count+1),xlab="ln(Counts+1)", ylab="Number of observations", main="Distribution of raw counts")
hist(flounder_long$count,xlab="Density (counts/m^2)", ylab="Number of observations", main="Distribution of raw densities")
hist(log(flounder_long$density+1),xlab="ln(Density+1)", ylab="Number of observations", main="Distribution of log-transformed densities")
```
...
```{r glounderGLMmultiplicative}
flounderGLM8<-MASS::glm.nb(flounder_long$count ~ 1+ flounder_long$Season * flounder_long$seagrass_cover * flounder_long$size + flounder_long$towArea)
...
```{r flounderZeroInflated}
m0<-zeroinfl(flounder_long$count ~ 1,
             dist="negbin")
m1<-zeroinfl(count ~ 1+ seagrass_cover*Season  + size +towArea,
             dist="negbin", data=flounder_long)
...
aic<-AIC(m0,m1,m2)
aic<-aic[order(aic[,2]),]
kable(aic,"latex",booktabs=TRUE)
```
```{r plotFlounderFittedResid}
plot(m1$fitted.values,m1$residuals,xlab="Fitted",ylab="Residuals")
```
```{r modelSummaryflounder}
summary(m1)
sumM1<-summary(m1)
sumM1_counts<-sumM1$coefficients$count
colnames(sumM1_counts)<-paste0(colnames(sumM1_counts),"_Counts")
sumM1_zeroes<-sumM1$coefficients$zero
colnames(sumM1_zeroes)<-paste0(colnames(sumM1_zeroes),"_Zeroes")
sumM1_zeroes<-rbind(sumM1_zeroes,rep(NA,4))
rownames(sumM1_zeroes)[nrow(sumM1_zeroes)]<-"Log(theta)"

m1sumcomb<-cbind(sumM1_counts,sumM1_zeroes)
write.csv(m1sumcomb,"FlounderModelResults.csv",row.names = TRUE,quote=FALSE)
```


```{r plotFlounderResids}
par(mfrow=c(1,5))
...
```{r flounderPredictedvsObs, fig.cap="Predicted counts compared to observed counts. The model does well for the smaller counts but performs poorly for large count observations."}
...
```
```{r flounder logPredObs, fig.cap="Log-transformed observed counts compared to predicted counts, only showing non-zero counts."}
...
I'm content with this level of fit, so we'll move forward with actually interpreting the analysis.


Get the predicteds:

```{r FlounderPredicteds}
combos <- data.frame(seagrass_cover = rep(c("bare","sparse", "patchy", "dense"), each=16), 
                      Season = rep(c("Winter","Spring","Summer","Fall"),4),
                     size = rep(levels(flounder_long$size),each=4),
                     towArea = mean(flounder_long$towArea),
                     stringsAsFactors = TRUE)
combos$seagrass_cover<-factor(combos$seagrass_cover,
                              levels=c("bare","sparse", "patchy", "dense"))
combos$Season<-factor(combos$Season,
                              levels=c("Winter","Spring","Summer","Fall"))
combos$size<-factor(combos$size,levels=levels(flounder_long$size))

## store the predicted number of flounder caught for each set
pp.combos <- cbind(combos, predict(m1, combos))

flounderPredicteds <- reshape2::melt(pp.combos, id.vars = c("seagrass_cover", "Season","size","towArea"), value.name = "counts")
flounderPredicteds<-flounderPredicteds[,!colnames(flounderPredicteds) %in% "variable"]
```



To get confidence intervals, we'll use bootstrapping, as suggested [on StackOverflow](https://stackoverflow.com/questions/58002334/how-to-add-confidence-intervals-for-a-zero-inflated-negative-binomial-function-i).

```{r flounderCIs}
stat <- function(df, inds) {
    model <- formula(count ~ 1+ seagrass_cover*Season  + size +towArea)
    predict(
        zeroinfl(model, dist = "negbin", data = df[inds, ]),
        newdata = combos)
}
```
```{r flounderPredictedsCIs, eval=FALSE}
set.seed(2018)
res <- boot(flounder_long, stat, R = 100)
CI <- setNames(as.data.frame(t(sapply(1:nrow(combos), function(row)
    boot.ci(res, conf = 0.95, type = "basic", index = row)$basic[, 4:5]))),
    c("lower", "upper"))
flounderPredicteds<-dplyr::bind_cols(flounderPredicteds,CI)
# can't have negative counts, so truncate those CIs:
flounderPredicteds$lower[flounderPredicteds$lower<0]<-0
write.csv(flounderPredicteds,"flounderPredicteds.csv",row.names = FALSE,quote=FALSE)
```
```{r readFlounderPredicteds}
flounderPredicteds<-read.csv("flounderPredicteds.csv",
                             stringsAsFactors = TRUE)
```
...
seagrassPlot<-function(flounderMeanCnts,flounderSEMCnts,cover){
  ...
  for(size in levels(flounderMeanCnts$SizeClass)){
    ...
    points(xs,
     ...
     points(xs,
           subMean$value,type='l',col=size_cols[cnt+1],lwd=2,lty=(cnt+1))
...
PlotFlounderProbs<-function(predictedProbs, sgrs,
  ...
  if(ymax==0) ymax<-round(max(dat$upper,na.rm=TRUE)+0.5)
  plot(counts~as.numeric(Season),
       data=dat[which(dat$size=="below10mm"),],
       pch=seagrassPs[1],
       cex=2,
       axes=FALSE,
       bty='L',
...
```{r flounderPredicteds,fig.width=6.39,fig.height=4.78}

par(mfcol=c(1,4),xpd=TRUE,mar=c(2,2,2,2),oma=c(3,4,3,2))
fp<-lapply(levels(flounderPredicteds$seagrass_cover),
           PlotFlounderProbs,
           predictedProbs=flounderPredicteds,
           seagrassCs=size_cols,
           seagrassPs=15:19,
           xaxis=FALSE)

l<-outer_legend("top",
             legend=levels(flounderPredicteds$size),
             col=size_cols,
             pch=15:19,
             lty=1:4,
             lwd=2,
             ...
             ncol=4,
             cex=1.5)
mtext("Predicted # Flounder per Tow",2,outer=TRUE,line=1.75)
...
png("../figs/flounderObsPred.png", height=5,width=9,res = 300,units='in')

par(mfrow=c(2,4), oma=c(8,1,2,1), mar=c(2,5.5,1,1),xpd=TRUE)
...
mtext("Predicted density",2,outer=FALSE,padj=-46.5,cex=1.25)
...
outer_legend("bottom",bty='n',
             pch=15:18,col=size_cols,levels(flounderMeanCnts$SizeClass),
             ncol=4,cex=2,lwd=3,lty=1:4,pt.cex=3)
dev.off()
```


```{r saveFlounderFig, eval=FALSE}
flounderfig<-image_convert(image_read("../figs/flounderObsPred.png"), "png")
flounderSeagrass<-image_append(c(flounderfig,
                                 seagrassPhotos), 
                               stack = TRUE)
image_write(flounderSeagrass,
            path = "../figs/FlounderFigure.png",
            format = "png")
```
...
```
```{r}
glmSumm<-summary(ageGLM4)
pipeTab<-t(glmSumm$coefficients)
colnames(pipeTab)<-paste0(colnames(pipeTab),"_coef")
pipeTab<-cbind(pipeTab,t(glmSumm$standard.errors))
colnames(pipeTab)[3:4]<-paste0(colnames(pipeTab)[3:4],"_se")
#pipeTab<-exp(pipeTab)
write.csv(pipeTab,"pipefishMultinomTable.csv",row.names = TRUE,quote=FALSE)
```

All of the predictions, when categorized as a count, are for an absence.

```{r pipefishPredicteds}
combos <- data.frame(seagrass_cover = rep(c("bare","sparse", "patchy", "dense"), each=8), 
                      Season = rep(c("Winter","Spring","Summer","Fall"),4),
...
```{r pipefishFitEffects}

fit.eff<-Effect(c("seagrass_cover","Season","species"), ageGLM4,se=list(level=0.99))
...
```{r pipefishProbabilities,fig.width=6.39,fig.height=4.78}

par(mfcol=c(2,2),xpd=TRUE,mar=c(2,2,2,2),oma=c(3,4,3,2))
sp<-lapply(c("adult","juvenile"),
           function(age, labels){
             PlotPipefishProbs(age, 
                               predictedProbs=pipePivot,
                               spp="stigmatopora",
                               seagrassCs=seagrassCs,
                               seagrassPs=seagrassPs)
             legend("topright",labels[age],bty='n',cex=2)
           }, labels=c(adult="A",juvenile="B")
           )
sl<-lapply(c("adult","juvenile"),
           function(age,labels){
             PlotPipefishProbs(age,
                               predictedProbs=pipePivot,
                               spp="leptonotus",
...
mtext("adults   ",2,outer=TRUE,line=-3,las=1,padj=-15,font=2)
mtext("juveniles",2,outer=TRUE,line=-3,las=1,padj=2,font=2)
...
```{r DuvauchelleTrendsPlot, eval=FALSE}
