...
PlotFlounderProbs<-function(predictedProbs, sgrs,
  ...
  ymin<-0
  ymax<-round(max(dat$upper,na.rm=TRUE)+0.5)
  plot(counts~as.numeric(Season),
...
```{r flounderPredicteds,fig.width=639,fig.height=478}
...
l<-outer_legend("top",
             levels(flounderPredicteds$size),
             col=size_cols,
             pch=15:19,
             lty=1:4,
             lwd=2,
...
png("../figs/flounderObsPred.png",width = 1512, height = 600, res = 96)

par(mfrow=c(2,4), oma=c(5,0,2,1), mar=c(2,5.5,1,1),xpd=TRUE)
...
fp<-lapply(levels(flounderPredicteds$seagrass_cover),
           ...
           seagrassPs=15:18,
           xaxis=FALSE)
mtext("Predcited density",2,outer=FALSE,padj=-73.5,cex=1.25)
...
mtext("Observed number",2,outer=FALSE,padj=-73.5,cex=1.25)

outer_legend("top",bty='n',
