...
[This tutorial](https://stats.idre.ucla.edu/r/dae/zinb/) has an example of how to get confidence intervals for parameters using bootstrapping. 

```{r, eval=FALSE}
library(boot)

cnts<-dput(round(coef(m1, "count"), 4))
zros<-dput(round(coef(m1, "zero"), 4))

f <- function(data, i) {
  require(pscl)
  m <- zeroinfl(count ~ 1+ seagrass_cover*Season  + size +towArea,
    data = data[i, ], dist = "negbin",
    start = list(count = cnts, 
                 zero = zros))
...
set.seed(10)
(res <- boot(flounder_long, f, R = 1200, parallel = "no", ncpus = 1))
```
```{r, eval=FALSE}
...
```
```{r}
kable(parms,"latex",booktabs=TRUE)
```

Can get the incident risk ratio and odds ratio for the zero inflation model
