...
```{r}
pipefishGLM1<-glm(pipefishCnts$count ~ 1+ as.factor(pipefishCnts$seagrass_cover)/as.factor(pipefishCnts$Season)  * pipefishCnts$size * pipefishCnts$towArea,family="poisson")
pipefishGLM2<-glm(pipefishCnts$count ~ 1+ as.factor(pipefishCnts$seagrass_cover)/as.factor(pipefishCnts$Season)  * pipefishCnts$size + pipefishCnts$towArea,family="poisson")
pipefishGLM3<-glm(pipefishCnts$count ~ 1+ as.factor(pipefishCnts$seagrass_cover)+as.factor(pipefishCnts$Season)  + pipefishCnts$size + pipefishCnts$species + pipefishCnts$towArea,family="poisson")

anova(pipefishGLM1,pipefishGLM2,pipefishGLM3,test="Chisq")
AIC(pipefishGLM1,pipefishGLM2,pipefishGLM3)
```
```{r}
par(mfrow=c(2,2))
plot(pipefishGLM3)
...
```
```{r}
chisq.test(pipefishCnts$size,predict(ageGLM4))
```

Well, crap, all of the predictions are for an absence.
...
```{r}
PlotPipefishProbs<-function(comboMelt, spp,stt,
                            seagrassCs, seagrassPs){
  dat<-comboMelt[which(comboMelt$species==spp &
                       comboMelt$variable==stt),]
  ymin<-min(dat$probability)-0.05
  if(ymin<0) ymin<-0
  ymax<-max(dat$probability)+0.05
  if(ymax>1) ymax<-1
  plot(probability~as.numeric(Season),
       ...
       col=seagrassCs[1],
       ylim=c(ymin,ymax))
  axis(1,at=1:4,labels=levels(comboMelt$Season),pos=ymin,
       cex.axis=1.5,lty=0)
  axis(2,las=1,cex.axis=1.5)
  
  lapply(2:length(levels(dat$seagrass_cover)),function(i){
...
sp<-lapply(levels(comboMelt$variable),
           PlotPipefishProbs,
           comboMelt=comboMelt,
           spp="stigmatopora",
           seagrassCs=seagrassCs,
           seagrassPs=seagrassPs)
sl<-lapply(levels(comboMelt$variable),
           PlotPipefishProbs,
           comboMelt=comboMelt,
           spp="leptonotus",
           seagrassCs=seagrassCs,
...
l<-outer_legend("bottom",
             levels(comboMelt$seagrass_cover),
             col=seagrassCs,
             pch=seagrassPs,
             lty=1:4,
             lwd=2,
...
mtext("absent",4,outer=TRUE,line=-5,las=1,padj=-19)
mtext("adults",4,outer=TRUE,line=-5,las=1,padj=-4.5)
mtext("juveniles",4,outer=TRUE,line=-5,las=1,padj=8.5)
