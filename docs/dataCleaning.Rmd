...
# get only the ones from duvauchelle
duv_traits<-body_dat[body_dat$LocationName=="Duvauchelle Bay",]
```

```{r TotalSNiCnts}
cleandat<-read.csv("../data/cleanedCommunity.csv")
sni_dat<-cleandat[,c("Day","towArea","Season","seagrass_cover",
                   grep("nigra",colnames(cleandat),value = TRUE))]
...
Check to make sure that all of the data match up -- community ecology counts and measurements:
```{r CheckMeasurementsVsCnts}
library(lubridate)
...
sni_num[is.na(sni_num)]<-0
sni_num$unmeasured<-sni_num$totalN-sni_num$measuredN
```

There are `r sum(sni_num$unmeasured,na.rm=TRUE)` observations that are also not in the measurements file -- this is good!
...
# fill in sizes of unmeasured juveniles
