...
I've added the calculation of pairwise turnover to the hierDiversity R code. I did so by incorporating Zach's functions from the script `diversityFunctionsNGBA.R` (which was associated with the Bayes paper) into the `div.part` and `dz` functions in hierDiversity. But now I want to test to make sure that the function is behaving as expected, so I'll test it using the simulations/data Zach used for the Bayes paper (from `PairwiseBetaSource.R` downloaded as an Appendix). 
...
### Create a giant list of results for plotting
truVal <- list(notPWTrue = notPWTrue, turn=turn, 
               biasLocal=biasLocal,biasRegion=biasRegion, 
               PWTrue=PWTrue, pwTurn = pwTurn , 
               pwLocal=pwLocal, pwRegion=pwRegion)
