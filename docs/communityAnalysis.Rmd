...
seagrassCond<-capscale(duvDetrend~ys$SeagrassCover + Condition(ys$Season) +
                         Condition(X) + 
                         Condition(Y) +
                         Condition(MEM6) +
                         Condition(MEM3) +
                         ...
                         Condition(MEM45) +
                         Condition(MEM16) +
                         Condition(MEM28),data=spaceAxes)
seasonCond<-capscale(duvDetrend~ys$Season + Condition(ys$SeagrassCover) + 
                       Condition(X) + 
                         Condition(Y) +
                         Condition(MEM6) +
                         Condition(MEM3) +
...
plotSpp<-as.data.frame(scrs[which(abs(scrs[,1])>=1 | abs(sppScrs[,2])>=1),])
...
ordiellipse(globalInt,ys$Season,col=seasonCols[levels(ys$Season)],lwd=5,choices=c(1,3))
...
ordiellipse(globalInt,ys$SeagrassCover,col=seagrassCs[levels(ys$SeagrassCover)],lwd=5,choices=c(1,3))
arrows(x0=rep(0,nrow(plotSpp)),
       y0=rep(0,nrow(plotSpp)),
       x1=plotSpp[,1]*0.1,
       y1=plotSpp[,2]*0.1,
...
# 4th axis
plotSpp<-as.data.frame(scrs[which(abs(scrs[,1])>=1 | abs(sppScrs[,3])>=1),])
...
ordiellipse(globalInt,ys$Season,col=seasonCols[levels(ys$Season)],lwd=5,choices=c(1,4))
...
ordiellipse(globalInt,ys$SeagrassCover,col=seagrassCs[levels(ys$SeagrassCover)],lwd=5,choices=c(1,4))
arrows(x0=rep(0,nrow(plotSpp)),
       y0=rep(0,nrow(plotSpp)),
       x1=plotSpp[,1]*0.1,
       y1=plotSpp[,3]*0.1,
...
plotQs<-function(qdat,levs,cols,pchs,makeLeg=TRUE,ylims=NULL,SEM=TRUE){
  ...
  plot(0:2,seq(0,max(qdat$mu),length.out = 3),
     ...
     axes=FALSE,
     cex.lab=2)
  axis(1, pos=0,at=-1:2,cex.axis=2)
...
par(mfrow=c(2,3),oma=c(1,1.5,1,0),mar=c(4,4,2,2),xpd=TRUE)
plotQs(seagrassQ,levels(combdat$SeagrassCover),seagrassCs,seagrassPs,FALSE,ylims=c(0,20))
legend("topright",
       ...
       ncol=1,
             bty='n',
             cex=2)
mtext("observed alpha diversity",2,outer=FALSE,cex=1.5,line=3.5)
plotQs(seasonsQ,levels(combdat$Season),seasonCols,seasonPchs,FALSE,ylims=c(0,20))
legend("topright",
...
plotQs(rarefied_summaries[rarefied_summaries$level %in% "Duvauchelle" &
       ...
       "Duvauchelle",c(Duvauchelle="#36454f"),c(Duvauchelle=19),FALSE,ylims=c(0,20))
legend("top","Overall",col="#36454f",pch=19,bty='n',cex=2)
...
plotQs(seagrassQ,levels(combdat$SeagrassCover),seagrassCs,seagrassPs,FALSE,ylims=c(0,20))
mtext("rarefied alpha diversity",2,outer=FALSE,cex=1.5,line=3.5)
plotQs(seasonsQ,levels(combdat$Season),seasonCols,seasonPchs,FALSE,ylims=c(0,20))
...
seagrassQ<-actualQdat[actualQdat$level %in% levels(combdat$SeagrassCover) &
                               actualQdat$diversity == "pwTurnover",]
par(mfrow=c(2,3),oma=c(1,1.5,1,0),mar=c(4,4,2,2),xpd=TRUE)
plotQs(seagrassQ,levels(combdat$SeagrassCover),seagrassCs,seagrassPs,FALSE,ylims=c(0,0.8))
legend("bottom",
       levels(combdat$SeagrassCover),
       col=seagrassCs,
       pch=seagrassPs,
       ncol=1,
             bty='n',
             cex=2)
mtext("observed pairwise turnover",2,outer=FALSE,cex=1.5,line=3.5)
plotQs(seasonsQ,levels(combdat$Season),seasonCols,seasonPchs,FALSE,ylims=c(0,0.8))
legend("topright",
       levels(combdat$Season),
       col=seasonCols,
       pch=seasonPchs,
...
plotQs(rarefied_summaries[rarefied_summaries$level %in% "Duvauchelle" &
       ...
       "Duvauchelle",c(Duvauchelle="#36454f"),c(Duvauchelle=19),FALSE,ylims=c(0,0.8))
legend("top","Overall",col="#36454f",pch=19,bty='n',cex=2)
...
plotQs(seagrassQ,levels(combdat$SeagrassCover),seagrassCs,seagrassPs,FALSE,ylims=c(0,0.8))
mtext("rarefied pairwise turnover",2,outer=FALSE,cex=1.5,line=3.5)
plotQs(seasonsQ,levels(combdat$Season),seasonCols,seasonPchs,FALSE,ylims=c(0,0.8))
