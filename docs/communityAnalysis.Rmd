...
```{r bestSelMEMs,fig.cap="Regions of Duvauchelle Bay that had the highest eigenvalue in the constrained axes of the global model, with the weightings of each point in the dbMEM shown in grey, with size based on its importance. These grey points are the ones that contributed the most variation to the spatial component of the analysis, and whose variation was partitioned out in the analysis of the global model.", fig.height=4}
selMEMs<-grep("MEM",bestCaps,value=TRUE)
par(mfrow=c(1,length(selMEMs)))
apply(spaceAxes[,selMEMs],
      2,
      function(weights,pts){
        plot(pts,cex=0.5,pch=19)
        points(pts,
             cex=weights,
             col=alpha("dark grey",weights*2),
             bg=col=alpha("dark grey",weights),
             pch=21)
      },pts=duvXY)
        text(x=10,y=1350,xpd=TRUE,
             "West")
        text(x=900,y=1300,xpd=TRUE,
             "East")
        text(x=-150,y=1350,xpd=TRUE,
             "South")
        text(x=-150,y=1800,xpd=TRUE,
             "North")
      },pts=duvXY,colMems=MEMCols)
```
