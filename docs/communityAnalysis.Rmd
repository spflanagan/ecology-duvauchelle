...
```{r}
xydbmem<-as.data.frame(dbmem(duvXY,silent=FALSE,thresh=50))
...
combine this with the space axes 

```{r}
spaceAxes <- cbind(duvXY, xydbmem[, attr(forwardSpace$terms, "term.labels")])
```

Use both environmental variables (separately) in the variance partitioning

```{r}
vp2<-varpart(duvDetrend,ys$SeagrassCover,ys$Season,spaceAxes)

plot(vp2, digits=2, bg=c("green", "red", "blue"), Xnames = c("seagrass", "season","spatial"))
```
