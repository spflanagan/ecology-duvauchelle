...
dat<-read.csv("../data/MonthlyRawSampling_Duvauchelle_2020.csv")
dat$Day<-as.Date(
  paste0(
    gsub("^(\\d)$","0\\1",dat$Date),
    gsub("^(\\d)$","0\\1",dat$Month),
    rawdat$Year),
  format = "%d%m%Y")
```
...
```{r PCoA,fig.height=7,fig.width=7,dpi=300}
colors<-gsub(".*_(\\w+$)","\\1",colnames(abund))
colors[colors=="Grass"]<-"green"
colors[colors=="Bare"]<-"brown"
library(vegan)
mod <- capscale(t(abund)~1, dist="bray")
biplot(mod)
```
